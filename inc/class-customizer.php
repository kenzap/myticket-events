...
function myticket_events_customizer_register( $wp_customize ) {
    ...
    require_once( MYTICKET_PATH . 'inc/class-customizer-repeater-control.php' );
    ...
    //enable calendar
    $wp_customize->add_setting( 'myticket_calendar', array(
                                'sanitize_callback' => 'myticket_events_sanitize_text',
                                ) );

    $wp_customize->add_control( 'myticket_calendar', array(
                                'label'     => esc_html__( 'Enable calendar', 'myticket-events' ),
                                'section'   => 'myticket_checkout_section',
                                'priority'  => 10,
                                'description' => esc_html__('Allow users to pick specific attendance day in cart calendar.', 'myticket-events' ),
                                'type'      => 'checkbox'
                                ) );
    ...
    // reservation time
    $wp_customize->add_setting( 'myticket_res_time', array(
                                'sanitize_callback' => 'myticket_events_sanitize_text',
                                'default' => 20
                                ) );

    $wp_customize->add_control( 'myticket_res_time', array(
                                'label'     => esc_html__( 'Max reservation time', 'myticket-events' ),
                                'section'   => 'myticket_checkout_section',
                                'priority'  => 10,
                                'description' => esc_html__( 'Maximum time in minutes needed for successful checkout. Not booked reservations are canceled after this time.', 'myticket-events' ),
                                'type'      => 'number'
                                ) );

    // checkout fields
    $wp_customize->add_setting( 'myticket_fields', array(
                                'sanitize_callback' => 'myticket_events_sanitize_text'
                                ) );

    $wp_customize->add_control( new MyTicket_Customizer_Repeater( $wp_customize, 'myticket_fields', array(
                                'label'   => esc_html__('Checkout fields','customizer-repeater'),
                                'section' => 'myticket_checkout_section',
                                'priority' => 20,
                                'description' => esc_html__( 'Add additional cart checkout input fields. Fields can be passed to PDF tickets or email notifications.', 'myticket-events' ),
                                'fields' => array(
                                    'title'     => array('title' => 'Field title', 'type'  => 'text',     'value'  => ''),
                                    'type'      => array('title' => 'Field type',  'type'  => 'select',   'value'  => '', 'choices' => array('text'=> 'Text', 'textarea'=> 'Textarea', 'email'=> 'Email', 'checkbox'=> 'Checkbox', 'note'=> 'Note') ),
                                    'key'       => array('title' => 'Field key',   'type'  => 'text',     'value'  => ''),
                                    'required'  => array('title' => 'Required',    'type'  => 'checkbox', 'value'  => ''),
                                )
                                ) ) );
}
