...
if ( $products->have_posts() ) : 
    ...
    <div class="schedule-content">
        <div class="kenzap-container <?php if("kenzap-sm"==$kenzapSize){$kenzapSize="kenzap-xs";} echo esc_attr($kenzapSize); ?>" style="max-width:<?php echo esc_attr($maxwidth);?>px">
            ...
            <?php if ( $myticket_show_header ) : sort($states); ?>
                ...
                <?php if ( count($states) ) {  
                    ...
                    foreach ( $states as $state ) {
                        ...
                        $day = date_i18n( 'd', intval( $state ) ); $day = preg_replace('/[^A-Za-z0-9]/', '', $day);
                        $month = date_i18n( 'M', intval( $state ) ); $month = preg_replace('/[^A-Za-z0-9]/', '', $month);
                        $year = date_i18n( 'Y', intval( $state ) ); $year = preg_replace('/[^A-Za-z0-9]/', '', $year);
						$daymonthyear = preg_replace('/[^A-Za-z0-9]/', '', $day.$month.$year);
                        if ( !in_array($daymonthyear, $monthsyearsdays, true) ){

                            array_push($monthsyearsdays, $daymonthyear);
                            array_push($dofs, $dof);
                            array_push($days, $day);
            ...
            <div class="event-tab-content">
                ...
                while ( $products->have_posts() ) : $products->the_post(); $meta = get_post_meta( get_the_ID() ); //print_r($meta); 
                    ...
                    $day = date_i18n( 'd', intval( $meta['myticket_datetime_start'][0] ) ); $day = preg_replace('/[^A-Za-z0-9]/', '', $day);
                    $month = date_i18n( 'M', intval( $meta['myticket_datetime_start'][0] ) ); $month = preg_replace('/[^A-Za-z0-9]/', '', $month);
                    $year = date_i18n( 'Y', intval( $meta['myticket_datetime_start'][0] ) );  $year = preg_replace('/[^A-Za-z0-9]/', '', $year);
                    ...
                    if ( $day.$month.$year != $daymonthyear ) :
                        ...
                        <div  class="tab-pane <?php if ( $daymonthyear == '' ){ echo 'active'; } $daymonthyear = $day.$month.$year; $daymonthyear = preg_replace('/[^A-Za-z0-9]/', '', $daymonthyear); ?>" id="tab<?php echo esc_attr($daymonthyear); ?>">
                    ...
                    if ( $stock == '' ){
                        ...
                        $stock = intVal($stock).' '.esc_attr__( ' Tickets Left', 'myticket' );
                    }
                            ...
                            <div class="tab-pane <?php if ( $ee == 1 ) { echo 'active'; } ?>" id="tab<?php echo esc_attr($e.'-hr'.$ee); ?>">
                                ...
                                <div class="full-event-info">
                                    <div class="full-event-info-header">
                                        ...
                                        <span class="event-date-info"><?php echo date_i18n( 'l, M d Y | '.get_option( 'time_format' ), intval( $meta['myticket_datetime_start'][0] ) ); ?></span>
