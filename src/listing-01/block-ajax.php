        ...
        <?php while ( $products->have_posts() ) : $products->the_post(); $meta = get_post_meta( get_the_ID() ); $i++; ?>
            ...
            <div class="search-result-item <?php if ( strlen( $meta['_sale_price'][0] ) > 0 ){ echo 'sale '; } if ( $meta['_stock_status'][0] == 'outofstock' ){ echo 'sold-out '; } ?>">
                ...
                <div class="kenzap-row">
                    ...
                    <?php if ( $ft_image ){ $thumb_id = get_post_thumbnail_id( get_the_ID() );     ?>
                        <div class="search-result-item-info kenzap-col-3">
                            <?php if($thumb_id){ ?><img class="ft-img" src="<?php echo wp_get_attachment_image_src( $thumb_id, 'woocommerce_single', true )[0]; ?>" />  <?php }else{ echo '&nbsp;'; } ?>
                        </div>
                    <?php } ?>

                    <div class="search-result-item-info kenzap-col-<?php echo ($ft_image)?6:9; ?>">
                        <h3><?php the_title();?></h3>
                        <ul class="kenzap-row">
