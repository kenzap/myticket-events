...
if ( $attributes['serverSide'] ){
	...
	?><img src="<?php echo MYTICKET_URL . 'assets/block_preview-01.jpg'; ?>" alt="<?php echo esc_attr__('block preview', 'myticket-events'); ?>" />
					...
					<div class="kenzap-row">
						<form>
							<div class="keyword kenzap-col-4">
								...
								<input type="text" class="form-control hasclear myticket-search-value" placeholder="<?php esc_attr_e( 'Search', 'myticket-events' ) ?>" value="<?php echo esc_attr(sanitize_text_field(isset($_GET['event'])?$_GET['event']:(isset($_GET['q'])?$_GET['q']:''))); ?>">
								<span class="clearer"><img src="<?php echo MYTICKET_URL; ?>assets/clear.svg" alt="<?php esc_attr_e( 'Remove search text', 'myticket-events' ) ?>"></span>
							</div>
							<?php global $wpdb;
							if ( strlen($attributes['filterLocations'])>0 ) {
								...
								$states = $wpdb->get_results(
									$wpdb->prepare(
										"SELECT DISTINCT meta_value FROM {$wpdb->postmeta} WHERE meta_key = %s ORDER BY meta_value",'myticket_title'
									)
								);
